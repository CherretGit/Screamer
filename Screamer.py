...
def select_image():
    file_path = filedialog.askopenfile()
    icon_path = file_path.name
    path_icon.delete(0, END)
    path_icon.insert(0, f'{icon_path}')
...
icon_image = CTkImage(Image.open(f'Assets/Settings.png'), size=(30, 30))
print(icon_image)
def settings_enter():
    global return_path
    ...
    global reicon_window_title
    reicon_window_title = customtkinter.CTkLabel(app, text='Path to image (In ico format)')
    reicon_window_title.place(x=170,y=130)
    global select_image_button
    select_image_button = customtkinter.CTkButton(master=app, text="Select Icon", height=35, width=100,command=select_image)
    select_image_button.place(x = 10, y=160)
    global path_icon
    path_icon = customtkinter.CTkEntry(app, width=200, height=35)
    path_icon.place(x = 150, y=160)
    global reicon_button
    reicon_button = customtkinter.CTkButton(master=app, text="ReIcon", height=35, width=100, command=reicon_window)
    reicon_button.place(x = 10, y=210)
    global reset_icon_button
    reset_icon_button = customtkinter.CTkButton(app, text='Reset Icon', height=35, width=100 , command=reset_window_icon)
    reset_icon_button.place(x=150,y=210)
    screamer_versinon_title = customtkinter.CTkLabel(app, text='Screamer V2.1')
...
def settings_exit():
    ...
    reicon_window_title.destroy()
    select_image_button.destroy()
    path_icon.destroy()
    reicon_button.destroy()
    reset_icon_button.destroy()
    new_window()
...
def reicon_window():
    image_path = (path_icon.get())
    if image_path == None:
        pass
    else:
        app.iconbitmap(f'{image_path}')

def reset_window_icon():
    app.iconbitmap(f'Assets/Icon.ico')
